...
namespace :lt do
  namespace :janitors do
    ...
    task import_raw_messages: :'lt:boot' do
      ...
      Analytics::Janitors::RawMessageImporter.new(LT.env.logger, 10000).import
    end
    ...
    task extract_page_visits: :'lt:boot' do
      ...
      Analytics::Janitors::VisitExtractor.new(LT.env.logger, 10000).extract
    end
    desc "Convert raw_messages to video_views"
    task extract_video_views: :'lt:boot' do
      require 'janitors/visualization_extractor'

      Analytics::Janitors::VisualizationExtractor.new(LT.env.logger, 10000).extract
    end
    ...
    task fill_video_youtube_information: :'lt:boot' do
      ...
      Analytics::Janitors::VideoMetadataExtractor.new(LT.env.logger, 10000, config).extract
    end
  end
