...
class HashWrapperTest < LTRedisTestBase
  def setup
    super

    @hash_wrapper =
      Analytics::Helpers::Redis::HashWrapper.new(connection, 'a_hash')
  end
...
class QueueWrapperTest < LTRedisTestBase
  def setup
    super

    @queue_wrapper =
      Analytics::Helpers::Redis::QueueWrapper.new(connection, 'a_queue')
  end
